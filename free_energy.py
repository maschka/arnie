...
def free_energy(seq, constraint=None, package='vienna_2', T=37, dangles=True, reweight=None, ensemble=True):
	...
	if package.lower().startswith('vienna'):
		...
		return pfunc(seq, package=package, T=T, dangles=dangles, constraint=constraint, reweight=reweight, return_free_energy=True)

	elif package.lower().startswith('contrafold'):
		Z_constrained = pfunc(seq, package=package, T=T, dangles=dangles, constraint=constraint)
